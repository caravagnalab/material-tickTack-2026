...
Samples <- readRDS("/orfeo/scratch/cdslab/antonelloa/material-tickTack-2026/PCAWG/Data/Samples.rds")
GENOME_SIZE_MB <- 3000
...
saveRDS(tmb_data, "/orfeo/scratch/cdslab/antonelloa/material-tickTack-2026/PCAWG/Data/TMB_data.rds")

TMB_data = readRDS("/orfeo/scratch/cdslab/antonelloa/material-tickTack-2026/PCAWG/Data/TMB_data.rds")

# s = TMB_data$cnaqc_path[1]
compute_fga = function(cnaqc_path){
  x = readRDS(cnaqc_path)
  fraction_of_genome_altered = x$basepairs_by_karyotype %>%
    filter(karyotype != "1:1") %>%
    pull(n) %>%
    sum()/3e9
  fraction_of_genome_altered
}
FGA_data = lapply(TMB_data$cnaqc_path %>% unique(), function(s){
  fga = compute_fga(s)
  df = TMB_data %>% filter(cnaqc_path == s) #%>% pull(sample)
  df$FGA = fga
  df
}) %>% Reduce(rbind, .)

saveRDS(FGA_data, file = "/orfeo/scratch/cdslab/antonelloa/material-tickTack-2026/PCAWG/Data/TMB_data.rds")
...
tmb_plot_data <- tmb_data %>%
  group_by(IntoGen_cancer_type, best_K) %>%
  mutate(sample_rank = dplyr::row_number(FGA)) %>%
  ungroup()
# Step 2: fixed width + ordered cancer levels by median n_snvs
cancer_levels <- tmb_plot_data %>%
  group_by(IntoGen_cancer_type) %>%
  summarise(median_fga = median(FGA), .groups = "drop") %>%
  arrange(median_fga) %>%
  pull(IntoGen_cancer_type)
FIXED_WIDTH <- tmb_plot_data %>%
  group_by(IntoGen_cancer_type) %>%
...
medians <- tmb_plot_data %>%
  group_by(IntoGen_cancer_type, best_K) %>%
  summarise(
    median_FGA = median(FGA),
    xmin       = min(x_pos) - 0.4,
    xmax       = max(x_pos) + 0.4,
    .groups    = "drop"
  )
# Step 8: plot
p <- ggplot(tmb_plot_data, aes(x = x_pos, y = FGA, color = factor(best_K), shape = class)) +
  geom_vline(xintercept = vline_positions, color = "grey60", linewidth = 0.4, linetype = "dashed") +
  geom_point(size = 1.5, alpha = 0.7) +
  geom_segment(
    data = medians,
    aes(x = xmin, xend = xmax, y = median_FGA, yend = median_FGA, color = factor(best_K)),
    linewidth = 0.8, inherit.aes = FALSE
  ) +
  scale_x_continuous(
    breaks = label_pos$x_center,
  ...
  #annotation_logticks(sides = "l") +
  labs(
    title = "Fraction of genome altered across Cancer Types",
    # subtitle = paste0("n = ", nrow(tmb_plot_data), " samples | WGS (~", GENOME_SIZE_MB, " Mb)"),
    x = "Cancer Type",
    y = "FGA (fraction of non diploid genome)"
    #caption = "Ordered by median n_snvs | Lines = median per k group"
  ) +
  theme_bw(base_size = 12) +
  theme(
...
p
ggsave("/orfeo/cephfs/scratch/cdslab/antonelloa/material-tickTack-2026/PCAWG/Plot/FGA_plot_bestk_ranked.pdf", p, width = 22, height = 7)

### Correlation FGA vs TMB
p1 = tmb_data %>% 
  ggplot(aes(x = FGA, y = log(TMB, base = 10), color = as.factor(best_K), shape = class)) +
  ...
  ylab("Tumor Mutational Burden (log-scale)")
  
ggsave("/orfeo/cephfs/scratch/cdslab/antonelloa/material-tickTack-2026/PCAWG/Plot/correlation_tmb_fga.pdf", 
       p1, width = 17, height = 10)
