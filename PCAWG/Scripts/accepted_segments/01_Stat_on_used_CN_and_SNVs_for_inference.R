...
# --- Per-sample karyotype summary ---
# Handles BOTH formats:
#  - old format: has Major/minor columns, karyotype must be derived
#  - new format: karyotype column already present (e.g. "2:0")
...
# --- Helper (OLD format): count mutations falling within each CNA segment ---
...
# --- Helper (NEW format): count how many entries in seg_assignment match
#     each segment's segment_id ---
add_seg_assignment_counts <- function(cna, seg_assignment) {
  
  if (length(seg_assignment) == 0 || nrow(cna) == 0) {
    cna$n <- 0L
    return(as_tibble(cna))
  }
  
  counts_tbl <- as_tibble(table(seg_assignment), .name_repair = "minimal") %>%
    setNames(c("segment_id", "n")) %>%
    mutate(segment_id = as.numeric(as.character(segment_id)),
           n = as.integer(n))
  
  cna %>%
    left_join(counts_tbl, by = "segment_id") %>%
...
for (idx in seq_len(nrow(Samples))) {
  ...
  cna_df <- fit$results_timing$data$accepted_cna
  
  # --- MODIFICATION (point 2): vectorized parsing of segment_name ---
  # Replaces rowwise() + strsplit() (row-by-row, slow) with a single
  # vectorized str_split_fixed() call, which parses all rows at once.
  # e.g. "chr5_137765178_180687907" -> from = 137765178, to = 180687907
  parts <- str_split_fixed(cna_df$segment_name, "_", 3)
  cna_df$from <- as.numeric(parts[, 2])
  cna_df$to   <- as.numeric(parts[, 3])
  
  # New format stores chr/from/to as character - coerce to correct types
  cna_df <- cna_df %>%
    mutate(
  ...
  if (!"n" %in% colnames(cna_df)) {
    if ("segment_id" %in% colnames(cna_df)) {
      # NEW format: count via seg_assignment matching segment_id
      seg_assignment <- fit$results_timing$data$input_data$seg_assignment
      if (is.null(seg_assignment)) seg_assignment <- integer(0)
      cna_df <- add_seg_assignment_counts(cna_df, seg_assignment)
    } else {
      # OLD format: count via mutation position overlap
      muts <- fit$mutations
      if (is.null(muts)) muts <- tibble()
      cna_df <- add_mutation_counts(cna_df, muts)
    }
  }
...
saveRDS(sample_list, "/orfeo/cephfs/scratch/cdslab/scocomello/material-tickTack-2026/PCAWG/Analysis_results/Cohort_stat/accepted_segments/sample_list.RDS")
multi_results <- run_multi_sample_summary(sample_list, karyotypes)
saveRDS(multi_results, "/orfeo/cephfs/scratch/cdslab/scocomello/material-tickTack-2026/PCAWG/Analysis_results/Cohort_stat/accepted_segments/multi_results.RDS")

sample_list <- readRDS("/orfeo/cephfs/scratch/cdslab/scocomello/material-tickTack-2026/PCAWG/Analysis_results/Cohort_stat/accepted_segments/sample_list.RDS")
multi_results <- readRDS("/orfeo/cephfs/scratch/cdslab/scocomello/material-tickTack-2026/PCAWG/Analysis_results/Cohort_stat/accepted_segments/multi_results.RDS")
...
raw_all <- imap_dfr(sample_list, function(df, sample_name) {
  if (!"karyotype" %in% colnames(df)) {
    df <- df %>% mutate(karyotype = paste(Major, minor, sep = ":"))
  }
  df %>%
    filter(karyotype %in% karyotypes)
})
...
pdf("/orfeo/cephfs/scratch/cdslab/scocomello/material-tickTack-2026/PCAWG/Analysis_results/Cohort_stat/accepted_segments/Timed_cohort_karyotype_all_cancer_types.pdf", width = 16, height = 12)
# pdf("~/old_home/re_gecip/cancer_pan/scocomello/material-tickTack-2026/Results/Cohort_stat/accepted_segments/test_1.pdf", width = 16, height = 12)
...
# render pheatmap to a raster grob (via temp PNG) - unchanged
...
pdf(paste0("/orfeo/cephfs/scratch/cdslab/scocomello/material-tickTack-2026/PCAWG/Analysis_results/Cohort_stat/accepted_segments/", "karyotype_by_cancer_type.pdf"), width = 16, height = 12)
...
for (ct in cancer_types_present) {
  ...
  # same raster-based capture here - unchanged
