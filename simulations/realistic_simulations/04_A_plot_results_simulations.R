...
# results_all_var <- readRDS(paste0(base_dir,"results_summary/results_ all_simulations_variational.rds"))
...
p1 <- ggplot(results_plot, aes(x = n_clusters, y = K, color = criterion)) +
  ...
  facet_grid(coverage ~ mutation_density + n_events, labeller = label_both) +
  labs(
...
ggplot(results_corr, aes(x = factor(n_clusters), y = correlation)) +
  ...
  facet_grid(purity + coverage ~ mutation_density + n_events, scales = "free") +
  labs(
