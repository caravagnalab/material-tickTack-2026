...
ttypes = list.files("/orfeo/cephfs/scratch/cdslab/antonelloa/material-tickTack-2026/PCAWG/Analysis_results/CNA_Signatures/Sigprofiler/")
...
for (t in ttypes){
  ...
  sigprofiler_dir = "/orfeo/cephfs/scratch/cdslab/antonelloa/material-tickTack-2026/PCAWG/Analysis_results/CNA_Signatures/Sigprofiler/"
  ...
  # ggsave("../REL_EXP_SIG_CNA_BREAST_PCAWG.pdf", height = 5, width = 8, plot = rel_exp)
  # ggsave("../PCA_SIG_CNA_BREAST_PCAWG.pdf", height = 5, width = 7, plot = final_plot2)
  
  # ── 2. Unsupervised Clustering ────────────────────────────────────────────────
  final_plot3 = clustering(df, class_colors) + ggtitle("K-means clustering")
  
  out_dir_plots = "../Analysis_results/CNA_Signatures/Plots/"
  
  if ( length(df %>% pull(class) %>% unique()) >= 2 ){
    ...
    patchwork::wrap_plots(
      final_plot1, final_plot3, final_plot2,
      design = "AA
              AA
              BB
              C#") %>% ggsave(filename = paste0(out_dir_plots, t, ".pdf"), height = 20, width = 12)
